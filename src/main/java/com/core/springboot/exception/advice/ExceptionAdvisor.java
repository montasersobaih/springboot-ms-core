package com.core.springboot.exception.advice;

import com.core.springboot.exception.BusinessException;
import com.core.springboot.exception.CustomServiceException;
import com.core.springboot.exception.GeneralException;
import com.core.springboot.exception.ServiceException;
import com.core.springboot.model.Error;
import com.core.springboot.model.ErrorResponse;
import com.core.springboot.utils.ExceptionUtils;
import com.core.springboot.utils.MessageResourceBundle;
...
import static com.core.springboot.exception.enumeration.ExceptionCategory.GENERAL;
import static com.core.springboot.utils.ExceptionUtils.buildErrorCode;
import static com.core.springboot.utils.ExceptionUtils.buildErrorDetails;
...
public abstract class ExceptionAdvisor {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, final Throwable ex) {
        ...
        for (String e : messages) {
            list.add(new Error(buildErrorCode(exception, "001"), e));
        }
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodArgumentTypeMismatchException ex) {
        CustomServiceException exception = new CustomServiceException(GENERAL, 400, "Method argument type mismatch");
        ...
        exception.getErrors().add(new Error(buildErrorCode(exception, "002"), errorMessage));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, BadRequestException ex) {
        ...
        CustomServiceException exception = new CustomServiceException(GENERAL, 400, "Bad request");
        exception.getErrors().add(new Error(buildErrorCode(exception, "003"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, InvalidFormatException ex) {
        ...
        ServiceException exception = new CustomServiceException(GENERAL, 400, "Invalid format");
        exception.getErrors().add(new Error(buildErrorCode(exception, "004"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MissingServletRequestParameterException ex) {
        ...
        ServiceException exception = new CustomServiceException(GENERAL, 400, "Missing servlet request parameter");
        exception.getErrors().add(new Error(buildErrorCode(exception, "005"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public final ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, NoHandlerFoundException ex) {
        ...
        ServiceException exception = new CustomServiceException(GENERAL, 404, "Resource not found");
        exception.getErrors().add(new Error(buildErrorCode(exception, "006"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public final ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodNotSupportedException ex) {
        ...
        ServiceException exception = new CustomServiceException(GENERAL, 405, "Method not supported");
        exception.getErrors().add(new Error(buildErrorCode(exception, "007"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public final ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, HttpRequestMethodNotSupportedException ex) {
        ...
        ServiceException exception = new CustomServiceException(GENERAL, 405, "Method not supported");
        exception.getErrors().add(new Error(buildErrorCode(exception, "008"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public final ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodNotFoundException ex) {
        ...
        ServiceException exception = new CustomServiceException(GENERAL, 404, "Method not found");
        exception.getErrors().add(new Error(buildErrorCode(exception, "009"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public final ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, WebExchangeBindException ex) {
        ServiceException exception = new CustomServiceException(GENERAL, 400, "Web exchange bind exception");
        exception.getErrors().add(new Error(buildErrorCode(exception, "010"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public final ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MaxUploadSizeExceededException ex) {
        ServiceException exception = new CustomServiceException(GENERAL, 400, "Max upload size exceeded");
        exception.getErrors().add(new Error(buildErrorCode(exception, "011"), ex.getMessage()));
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, HttpMessageNotReadableException ex) {
        ...
        if (matcher.find()) {
            ...
            exception.getErrors().add(buildErrorDetails(fieldName.concat(ex.getClass().getSimpleName()), path, exception));
            exception.setSource(getExceptionSource(fieldName, ex));
        } else {
            // the whole request payload is not in proper format
            exception.getErrors().add(buildErrorDetails(ex.getClass().getSimpleName(), exception));
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, ValidationException ex) {
        ...
        if (matcher.find()) {
            ...
            exception.getErrors().add(buildErrorDetails(exceptionName, fieldName, exception));
        }
        exception.setSource(ExceptionUtils.getDefaultProjectName());
        return handleException(request, exception);
    }
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, ConstraintViolationException ex) {
        ...
        List<Error> errorList = ex.getConstraintViolations().stream()
                .map(e -> buildErrorDetails(e.getMessageTemplate(), exception))
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodArgumentNotValidException ex) {
        ...
        List<Error> errorsList = allErrors.stream().map(
                e -> buildErrorDetails(e.getDefaultMessage(), ((FieldError) e).getField(), exception))
                .collect(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(Error::getErrorCode))))
                .stream().collect(Collectors.toList());
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, final ServiceException ex) {
        ...
        if (ex.getErrors().isEmpty()) {
            ex.getErrors().add(buildErrorDetails(ex.getClass().getSimpleName(), ex));
