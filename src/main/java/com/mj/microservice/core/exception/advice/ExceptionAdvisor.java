package com.mj.microservice.core.exception.advice;

import com.fasterxml.jackson.databind.exc.InvalidFormatException;
import com.mj.microservice.core.exception.BusinessException;
import com.mj.microservice.core.exception.CustomServiceException;
import com.mj.microservice.core.exception.GeneralException;
...
import com.mj.microservice.core.model.Error;
import com.mj.microservice.core.model.ErrorResponse;
import com.mj.microservice.core.utils.ExceptionUtils;
import com.mj.microservice.core.utils.MessageResourceBundle;
...
public abstract class ExceptionAdvisor {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, final Throwable ex) {
        ...
        for (String message : messages) {
            errors.add(new Error(ExceptionUtils.buildErrorCode(exception, "001"), message));
        }

        exception.setErrors(errors);
        exception.setSource(ExceptionUtils.getDefaultProjectName());

        ResponseEntity<ErrorResponse> errorResponse = handleException(request, exception);
        // log full exception
        logStackTrace(ex, errorResponse.getBody().getRequestId());
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodArgumentTypeMismatchException ex) {
        ...
        String formatedMessage = "%s data type mismatch, cannot convert \'%s\' to %s";
        String errorMessage = String.format(formatedMessage, ex.getName(), ex.getValue(), ex.getRequiredType().getName());
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, NoHandlerFoundException ex) {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodNotSupportedException ex) {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, HttpRequestMethodNotSupportedException ex) {
        ServiceException exception = new CustomServiceException(ExceptionCategory.GENERAL, 405, "Method not supported");
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodNotFoundException ex) {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, WebExchangeBindException ex) {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MaxUploadSizeExceededException ex) {
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, HttpMessageNotReadableException ex) {
        ...
        String errorMessage = ex.getMessage().replaceAll("\\n", "");
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, MethodArgumentNotValidException ex) {
        ...
        List<Error> errorsList = allErrors.stream()
                .map(e -> ExceptionUtils.buildErrorDetails(e.getDefaultMessage(), ((FieldError) e).getField(), exception))
                .collect(Collectors.toCollection(() -> new TreeSet<>(Comparator.comparing(Error::getErrorCode))))
                .stream()
                .collect(Collectors.toList());
    ...
    public ResponseEntity<ErrorResponse> handleException(HttpServletRequest request, final ServiceException ex) {
        ExceptionCategory category = ex.getCategory();

        String name = StringUtils.capitalize(category.getName());
        String code = category.getCode();
        String message = ex.getMessage();

        log.error("{} exception -> code: {}, message: {}", name, code, message, ex);
        ex.setUrl("");
        ...
        ErrorResponse eResponse = ErrorResponse.from(ex);
        HttpHeaders headers = ExceptionUtils.getExceptionHttpHeaders(ex);

        return new ResponseEntity<>(eResponse, headers, HttpStatus.resolve(ex.getStatusCode()));
    }
